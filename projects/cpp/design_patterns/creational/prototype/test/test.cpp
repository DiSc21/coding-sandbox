...
TEST(Projectile_, someMethodOnHit) // NOLINT
{
    ...
    std::set<std::shared_ptr<Projectile_>> projectiles = {projectile_factory.create(ProjectileFactory::Type::ARROW),
...
TEST(ProjectileFactory, create) // NOLINT
{
    ...
    uint8_t damage_sum =
        std::accumulate(projectiles.begin(), projectiles.end(), 0, [&](uint8_t sum, const std::shared_ptr<Projectile_> &contrib) {
